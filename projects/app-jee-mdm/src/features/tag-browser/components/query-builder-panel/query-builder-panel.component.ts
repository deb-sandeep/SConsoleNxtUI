...
// the tag-query tree editor + optional filters, and the Apply button. Both
// telescoping sub-panels — "+tag" (tag-picker-panel) and "Browse topics"
// (topic-picker-panel) — are rendered here too (always present, each gated
// on its own TagBrowserService flag internally) since they only ever open
// from within this panel.
@Component({
  selector: 'query-builder-panel',
  imports: [ TagQueryTreeNodeComponent, TagBrowserFiltersComponent, TagPickerPanelComponent, TopicPickerPanelComponent ],
